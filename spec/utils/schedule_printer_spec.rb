...
describe SchedulePrinter do
  ...
  describe "when printing a schedule with a GIA400 course, in group 2, from 8:00 to 11:00, on monday" do
    let(:course) { Period.on("monday", "Cours").from("8h00").to("11h00") }
  ...
  describe "when printing a schedule with a LOG120 labcourse, in group 3, from 11:10 to 16:17, on thursday" do
    let(:course) { Period.on("thursday", "Labo").from("11h10").to("16h17") }
    let(:group) { Group.new "LOG120", 3, [course] }
    let(:schedule) { [group] }
  ...
  describe "when printing a complicated schedule" do
    let(:course_1_1) { Period.on("monday", "Cours").from("13h30").to("17h00") }
    let(:course_2_1) { Period.on("thursday", "TP-Labo A").from("13h30").to("15h30") }
    let(:course_3_1) { Period.on("thursday", "TP-Labo B").from("15h30").to("17h00") }
    let(:group_1) { Group.new "GIA601", 2, [course_1_1, course_2_1, course_3_1] }

    let(:course_1_2) { Period.on("tuesday", "Cours").from("8h45").to("12h15") }
    let(:course_2_2) { Period.on("thursday", "TP").from("8h30").to("10h30") }
    let(:group_2) { Group.new "GPE450", 1, [course_1_2, course_2_2] }

    let(:course_1_3) { Period.on("wednesday", "Cours").from("18h00").to("21h30") }
    let(:course_2_3) { Period.on("monday", "Labo").from("18h00").to("20h00") }
    let(:group_3) { Group.new "LOG550", 1, [course_1_3, course_2_3] }

    let(:course_1_4) { Period.on("thursday", "Cours").from("18h00").to("21h30") }
    let(:course_2_4) { Period.on("tuesday", "TP/Labo").from("18h00").to("20h00") }
    let(:group_4) { Group.new "LOG619", 1, [course_1_4, course_2_4] }
